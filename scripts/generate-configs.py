...
def gen_inventory(cfg: dict, env: str) -> str:
    ...
    # nexus_apt_proxy — base Nexus URL emitted into all.vars so Phases 3+ roles can
    # construct per-repo URLs (e.g. {{ nexus_apt_proxy }}/repository/apt-proxy-trixie/).
    ...
    nexus_apt_proxy = f"http://{nexus_ip}:8081" if nexus_ip else ""
