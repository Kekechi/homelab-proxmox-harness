...
#                   (default: true)
#   NEXUS_APT_URL   Base URL of a Nexus apt proxy repo for the Ubuntu jammy suite,
#                   e.g. http://nexus.home.lab:8081/repository/ubuntu-jammy-proxy
...
#   https://cloud-images.ubuntu.com/jammy/YYYYMMDD/jammy-server-cloudimg-amd64.img
# Release dates are listed at: https://cloud-images.ubuntu.com/jammy/
#
...
if [[ "${INJECT_AGENT}" != "true" ]]; then
  ...
  virt-customize -a "${IMAGE_FILE}" \
    ...
    --run-command "echo \"deb ${NEXUS_AUTH_URL} jammy main universe\" > /etc/apt/sources.list" \
